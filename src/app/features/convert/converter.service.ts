...
export class ConverterService {
  private webpReady?: Promise<unknown>;
  ...
  async convert(source: ConversionSource, settings: ConversionSettings): Promise<ConversionResult> {
    ...
    try {
      ...
      if (settings.format === 'ico') {
        ...
        context.drawImage(source.image, (width - w) / 2, (height - h) / 2, w, h);
      ...
      if (settings.format === 'bmp') blob = packBmp(context.getImageData(0, 0, width, height));
        ...
        if (settings.format === 'webp' && blob.type !== format.mime) {
          ...
          await (this.webpReady ??= init({
            locateFile: (path: string) => `/assets/codecs/${path}`,
          }).catch((error: unknown) => {
            this.webpReady = undefined;
            throw error;
          }));
          blob = new Blob(
            [
              await encode(context.getImageData(0, 0, width, height), {
