import { outputDimensions } from '@features/hdr/utils/output-size';
import { imagePlacement } from '@shared/images/utils/image-geometry';
import { ImageSource } from '@shared/images/models/image-source.model';
import { Service } from '@angular/core';
import { embedICCProfile, embedPNGCICP } from '@features/hdr/engine/container';
import { encodeToPQ } from '@features/hdr/engine/encoder';
import type {
  EncodeWorkerRequest,
  EncodeWorkerResponse,
  EncodeWorkerResult,
} from '@features/hdr/engine/encoder-worker.models';
import { buildICCProfile, CICP } from '@features/hdr/engine/icc';
import {
  JpegExport,
  EncodeResult,
  EncodeSettings,
  EncodeStats,
  PixelImage,
} from '@features/hdr/models/bench.models';

import {
  MAX_IMAGE_PIXELS,
  MAX_IMAGE_SIDE,
  MAX_IMAGE_FILE_BYTES,
} from '@shared/images/constants/image-limits';

@Service()
export class ImageEncoderService {
  private readonly profile = buildICCProfile();

  async load(file: File): Promise<ImageSource> {
  ...
  async prepareJpeg(source: ImageSource, settings: EncodeSettings, signal: AbortSignal) {
  ...
  private async render(source: ImageSource, settings: EncodeSettings, signal: AbortSignal) {
    signal.throwIfAborted();
    const { width, height } = outputDimensions(source, settings);
